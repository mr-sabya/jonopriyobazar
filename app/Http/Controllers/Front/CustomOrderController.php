...
class CustomOrderController extends Controller
{
	...
	public function index()
	{
		...
		return view('front.custom.index', compact('shipping_address', 'billing_address', 'districts'));
	}
	...
	public function store(Request $request)
	{
		...
		if($request->custom == '' && !$request->hasFile('image')){
			return response()->json([
				'errors' => [
					'custom' => 'Please add custom list or Image',
				],
			]);
		}
		...
		if ($request->hasFile('image')) {
			...
			$filename = time().'-custom-order-'.$extension;
