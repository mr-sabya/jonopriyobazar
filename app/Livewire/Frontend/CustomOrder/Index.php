...
class Index extends Component
{
    ...
    // Logic for Dependent Dropdowns (Replacing CommonController AJAX)
    ...
    public function store()
    {
        ...
        // Validate Address if no existing shipping address
        ...
        // Custom list or Image validation
        if (empty($this->custom) && !$this->image) {
            $this->addError('custom', 'Please add custom list or upload an Image');
            return;
        }
        ...
        if (!$this->shipping_address) {
            ...
            $this->loadUserAddresses(); // Refresh addresses
        }
        ...
        // Handle Image Upload
        if ($this->image) {
            $filename = time() . '-custom-order-' . $this->image->getClientOriginalName();
            $this->image->storeAs('images', $filename, 'public_uploads'); // ensure you have 'public_uploads' disk or use public path
