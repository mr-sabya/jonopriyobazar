    ...
    <div class="row clearfix">
        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="header">
                    <h2><strong>Delivery</strong> Status </h2>
                </div>
                <div class="body">
...
<div class="modal fade" id="status_modal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content position-relative">
            <div id="status_modal_loader" class="loader-popup" style="display: none;">
                <img src="{{ url('backend/images/loader.gif')}}">
            </div>
            <div class="modal-header">
                <h4 class="title" id="status_modal_title">Add Statu</h4>
            </div>
            <form id="status_form" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <span style="color: red" id="slug_error"></span>
                    </div>
                </div>
...
<script>
    ...
    $('#status_table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('admin.deliverystatus.index') }}",
        },
        columns: [
        {
            data: 'DT_RowIndex',
    ...
    $('#add_new').click(function() {
        ...
        $('#name_error').html('');
        $('#slug_error').html('');
        $('#status_modal_title').text("Add New Status");
    ...
    $('#status_form').on('submit', function (event) {
        ...
        if ($('#action').val() == 'Add') {
            $.ajax({
                url: "{{ route('admin.deliverystatus.store') }}",
                method: "POST",
                ...
                dataType: "json",
                beforeSend: function(){
                    $('#status_modal_loader').show();
                },
                success: function (data) {
                    $('#status_modal_loader').hide();
                    if (data.errors) {
                        $('#name_error').html(data.errors.name);
                        $('#slug_error').html(data.errors.slug);
                    }
                    if (data.success) {
                        swal({
                            title: "Good job!",
        ...
        // -------------> for update <-------------------------------- 
        $.ajax({
            url: "{{ route('admin.deliverystatus.update') }}",
            method: "POST",
            data: new FormData(this),
            contentType: false,
            cache: false,
    ...
    $(document).on('click', '.edit', function () {
        ...
        // alert(id);
        $.ajax({
            url: "/control/delivery-status/" + id + "/edit",
            dataType: "json",
            beforeSend: function(){
                $('#status_modal_loader').show();
            },
            success: function (data) {
                $('#name').val(data.status.name);
                $('#slug').val(data.status.slug);
                $('#hidden_id').val(data.status.id);
                $('#action_button').html("Update");
                $('#status_modal_title').html("Update Status");
                $('#action').val("Edit");
                $('#status_modal_loader').hide();
            }
        })
