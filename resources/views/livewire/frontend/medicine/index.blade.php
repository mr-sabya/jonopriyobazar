        ...
        <div class="row justify-content-center">
            <div class="col-lg-10">
                ...
                <div class="text-center mb-5">
                    <h2 class="font-weight-bold text-dark">Medicine Order Form</h2>
                    <p class="text-muted mx-auto" style="max-width: 600px;">
                        মেডিসিন অর্ডার ফরমের মাধ্যমে আপনার প্রেসক্রিপশন-এর ছবি আপলোড করে ঔষধ অর্ডার করতে পারবেন
                    </p>
                ...
                <div class="row justify-content-center">
                    <div class="col-lg-9">
                        <form wire:submit.prevent="store">
                            ...
                            <div class="card border-0 shadow-sm mb-4 br-15">
                                <div class="card-body p-4">
                                    ...
                                    <div class="form-group">
                                        <label class="small font-weight-bold text-muted">Street Address</label>
                                        <input type="text" wire:model="street" class="form-control @error('street') is-invalid @enderror" placeholder="House, Road, Area...">
                            ...
                            @else
                            <div class="card border-0 shadow-sm mb-4 br-15 overflow-hidden text-dark">
                                <div class="bg-light p-3 border-bottom d-flex justify-content-between align-items-center">
                                    <p class="mb-0 small text-success font-weight-bold"><i class="fas fa-check-circle mr-1"></i> Delivery to saved address</p>
                                </div>
                                <div class="card-body p-4">
                                    <div class="row">
                                        <div class="col-md-6 border-right">
                                            <h6 class="small font-weight-bold text-muted mb-2 text-uppercase">Shipping To</h6>
                                            <p class="mb-1 font-weight-bold">{{ $shipping_address->name }}</p>
                                            <p class="small mb-0 text-muted">{{ $shipping_address->street }}</p>
                                            <p class="small text-muted">{{ $shipping_address->phone }}</p>
                                        </div>
                                        <div class="col-md-6 pl-md-4">
                                            <h6 class="small font-weight-bold text-muted mb-2 text-uppercase">Billing To</h6>
                            ...
                            <div class="d-flex align-items-center mb-3">
                                ...
                                <h5 class="mb-0 font-weight-bold">Prescription & Medicine List</h5>
                            </div>
                            ...
                            <div class="card border-0 shadow-sm mb-4 br-15">
                                <div class="card-body p-4">
                                    ...
                                    <div class="form-group mb-4">
                                        <label class="small font-weight-bold text-muted">Upload Prescription (Required)</label>
                                        <div class="custom-file-upload text-center p-4 border rounded @error('image') border-danger @enderror">
                                            <input type="file" wire:model="image" id="pres-upload" class="d-none">
                                            <label for="pres-upload" class="m-0 cursor-pointer">
                                                <i class="fas fa-file-medical fa-2x text-success mb-2"></i>
                                                <p class="mb-0 small font-weight-bold">Click to Upload Prescription Photo</p>
                                            </label>
                                            <div wire:loading wire:target="image" class="small text-primary mt-1">Uploading...</div>
                                            @if($image) <div class="small text-success mt-2 font-weight-bold">Selected: {{ $image->getClientOriginalName() }}</div> @endif
                                        </div>
                                        @error('image') <small class="text-danger font-weight-bold mt-1 d-block">{{ $message }}</small> @enderror
                                    </div>

                                    <div class="form-group mb-0">
                                        <label class="small font-weight-bold text-muted">Medicine List (Optional)</label>
                                        <textarea wire:model="custom" class="form-control br-10" rows="5" placeholder="1. Napa Extra - 2 strips&#10;2. Sergel 20mg - 1 box..."></textarea>
                                    ...
                                    <div class="payment-list">
                                        ...
                                        <label class="payment-box d-flex align-items-center {{ $payment_option == 'cash' ? 'active' : '' }}">
                                            ...
                                            <div class="flex-grow-1">
                                                <span class="d-block font-weight-bold small text-dark">Cash On Delivery</span>
                                                <span class="text-muted small">Pay when medicine arrives</span>
                                            </div>
                                        ...
                                        $walletReason = $user->is_hold == 1 ? "Wallet on hold" : ($user->is_expired == 1 ? "Wallet expired" : "Wallet inactive");
                                        @endphp
                                        <label class="payment-box d-flex align-items-center {{ $walletDisabled ? 'disabled' : ($payment_option == 'wallet' ? 'active' : '') }}">
                                            ...
                                            <div class="flex-grow-1">
                                                <span class="d-block font-weight-bold small {{ $walletDisabled ? 'text-muted' : 'text-dark' }}">Credit Wallet</span>
                                            </div>
                                            @if($walletDisabled)
                                        ...
                                        <label class="payment-box d-flex align-items-center {{ $referDisabled ? 'disabled' : ($payment_option == 'reffer' ? 'active' : '') }}">
                                            ...
                                            <div class="flex-grow-1">
                                                <span class="d-block font-weight-bold small {{ $referDisabled ? 'text-muted' : 'text-dark' }}">Refer Wallet</span>
                                            </div>
                            ...
                            <button type="submit" class="btn btn-success btn-block py-3 shadow font-weight-bold br-10" wire:loading.attr="disabled">
                                <span wire:loading.remove><i class="fas fa-check-circle mr-1"></i> Order Now</span>
                                <span wire:loading><i class="fas fa-spinner fa-spin mr-1"></i> Processing Order...</span>
                            </button>
                        </form>
                    </div>
                </div>
